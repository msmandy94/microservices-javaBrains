...
/*        CommandHelloWorld command = new CommandHelloWorld("someName");
        ...
        */
        ...
        List<CatalogItem> catalogItems = userRatings.getRatings().stream()
                .map(rating -> {
                    Movie movie = movieInfoService.getMovie(rating.getMovieId());
                    /* 2nd aproach..
    ...
    private List<CatalogItem> getUserCatalogFallBack(@PathVariable("userId") String userId) {
        return Collections.singletonList(new CatalogItem("getUserCatalogFallBack..no name", "getUserCatalogFallBack..no description", 0));
    }
